 ...
 */
export const getArtistDetails = async (artistIds, artistCounts = {}) => {
  if (!artistIds || artistIds.length === 0) {
    return { artists: [], genreStats: [], artistScatterData: [] };
  }
  ...
  try {
    ...
    const responses = await Promise.all(
      artistIdBatches.map(idList =>
        fetch(`https://api.spotify.com/v1/artists?ids=${idList}`, {
    ...
    const artistScatterData = allArtists.map(artist => ({
      name: artist.name,
      followers: artist.followers?.total || 0,
      popularity: artist.popularity || 0,
      trackCount: artistCounts[artist.name] || 0,
    }));
